...
return(
  <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-10 bg-primary`}>
    <div className="w-full flex justify-between max-w-7x1 mx-auto">
      ...
      <img src={logo} alt="logo"
       ...
       <p className="text-white text-[18px] font-bold cursor-pointer flex">Michael &nbsp;<span className="sm:block hidden">| Front-End Dev</span></p>
       </Link>
        ...
        <div className={`${!toggle ? "hidden":"flex"} p-6 black-gradient absolute top-20 right-0
         mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
          <ul className="list-none flex justify-end items-start flex-col gap-4 ">
            {navLinks.map((l) => {
              return(<li key={l.id}
              className={`${active === l.title ? "text-white" : "text-secondary"} font-poppins font-medium cursor-pointer text-[16px] `}
              onClick={()=>{
                 setActive(l.title);
                 setToggle(!toggle)
                 }}>
